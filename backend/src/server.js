...
const playbookScheduler = require('./playbook/schedulers/playbookScheduler');
...
async function startServer() {
  try {
    ...
    // Start playbook scheduler (market context every 5s, regime every 30s)
    if (process.env.ENABLE_PLAYBOOK_SCHEDULER !== 'false') {
      try {
        playbookScheduler.start();
        const evaluationWorker = require('./playbook/workers/evaluationWorker');
        evaluationWorker.start();
      } catch (err) {
        console.warn('[Playbook] Scheduler/worker failed to start:', err.message);
      }
    }
