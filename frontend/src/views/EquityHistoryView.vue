            ...
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
                        Track your account equity over time. Click on any day to
                        add or edit your equity amount.
                    </p>
                </div>
                        ...
                        <div class="mb-4">
                            ...
                            <div class="relative">
                                ...
                                <input
                                    ...
                                    class="block w-full pl-8 pr-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-primary-500 focus:border-primary-500 dark:bg-gray-700 dark:text-white"
                                    required
                                />
                            </div>
                        </div>
...
function updateCalendarData() {
    ...
    for (let i = 0; i < 42; i++) {
        ...
        if (dateStr === "2025-07-09") {
            ...
            console.log(
                "  available dates:",
                equitySnapshots.value.map((s) => s.snapshot_date),
            );
        }
...
function updateMonthlyStats() {
    ...
    monthStartEquity.value =
        monthlyEquities.length > 0 ? monthlyEquities[0].equity_amount : null;
    monthEndEquity.value =
        monthlyEquities.length > 0
            ? monthlyEquities[monthlyEquities.length - 1].equity_amount
            : null;
...
async function saveEquity() {
    ...
    try {
        ...
        if (selectedDayEquity.value) {
            // Update existing
            await api.put(
                `/equity/snapshots/${selectedDayEquity.value.id}`,
                payload,
            );
        } else {
            // Create new
...
function deleteEquity() {
    ...
    showDangerConfirmation(
        "Delete Equity Entry",
        "Are you sure you want to delete this equity entry?",
        async () => {
            saving.value = true;
...
async function loadEquitySnapshots() {
    try {
        ...
        calendarStartDate.setDate(
            calendarStartDate.getDate() - firstDay.getDay(),
        );

        const lastDay = new Date(year, month + 1, 0);
        const calendarEndDate = new Date(lastDay);
        calendarEndDate.setDate(
            calendarEndDate.getDate() + (6 - lastDay.getDay()),
        );
        ...
        console.log(
            "Equity snapshots details:",
            equitySnapshots.value.map((s) => ({
                id: s.id,
